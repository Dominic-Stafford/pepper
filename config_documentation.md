...
- `exit_on_failed_jobs`: Optional, string specifying whether to exit if a single job fails retries number of times. Can be either `"all"` (to exit on any such failures, recommended if using pre-computed lumifactors), `"data"` (to exit only on data failures, can be useful if using post-computed lumifactors) or `"none"` (to not exit; the user will need to account for any such failures)
...
- `mc_lumifactors`: Path to JSON file produced by the `compute_mc_lumifactors.py` script or false, which indicates pepper should compute lumifactors after running (useful if it is not possible to reliably run over all MC samples).
- `pileup_reweighting`: Optional, path to the pileup weights generated by the `compute_pileup_weights.py` script or list for correctionlib. For correctionlib the first element is the path to the json, the second is the key for the corrections (e.g. "Collisions17_UltraLegacy_goldenJSON"), and the third, which can be ommited, is a dictionary for potential extra parameters required by the corrections.
...
- `electron_sf`: Optional, array of arrays, the inner ones containing exactly three elements. For SFs in ROOT files, the first element shuld be a path to a scale factor ROOT file provided by the electron POG, the second the name of the histogram to use inside the ROOT file, usually "EGamma_SF2D", and the third an array specifying the order of the axes in the histogram; axes normally are "eta" and "pt" but can also be other particle attributes. For correctionlib SFs, the first element should be the path to the correctionlib json (can be compressed), the second the name of the correction (usually "UL-Electron-ID-SF"), and the third the additional information which should be passed to the correction but is not available as a table column (normally "year" and "WorkingPoint"). It is possible to specify different correction-keys for different $p_t$-ranges (i.e. if two different corrections are to be applied below and above $20\;\text{GeV}$) by providing a dict of the form: `"WorkingPoint": {"Reco20to75": [20, 75], "RecoAbove75": [75, "inf"]}`.
- `muon_sf`: Optional, array of arrays, the inner ones containing exactly three elements. For SFs in ROOT files, the first one should be a path to a scale factor ROOT file provided by the muon POG, the second the name of the histogram to use inside the ROOT file, the third an array specifying the order of the axes in the histogram. Axes normally are "abseta" and "pt" but can also be other particle attributes. For correctionlib SFs, the first element should be the path to the correctionlib json (can be compressed), the second the name of the correction, and the third the additional information which should be passed to the correction but is not available as a table column (normally "year").
...
- `reapply_jec`: Optional, boolean. If true, the jet energy correction given in `jme_correctionlib_corrections` or given in `jet_correction` (for MC) and `jet_correction_data` will be used to correct the jets. NanoAOD always comes with some jet energy correction applied. These will be undone in order to do the correction. Reapplying is only needed if a different version of jet energy corrections is required that the one in NanoAOD.
- `jme_correctionlib_corrections` Starting in Run 3, this is the prefered way to define JME corrections using a single correctionlib file. Needed if `reapply_jec` is true. Must point to a dictionary with the following keys:
...
- `jet_correction_data`: Needed if `reapply_jec` is true. Array of paths to AK4PFchs jet energy correction TXT files for data provided by the JetMET POG. These are usually L1, L2 and L3 corrections. Can be optionally split by datataking era, in which case it should be a dict mapping eras to arrays of paths.
...
- `weight`: Optional, data picker, cannot be used in conjuction with `weight_factor`. Specifies a custom event weight using a data picker, which will be used instead of the event weight. Systematics will not be produced when using this option (since it is intended for data-driven methods). Can either be one value per event, or have the same shape as the fill.
- `weight_factor`: Optional, data picker, cannot be used in conjuction with `weight`. Specifies a custom event weight factor using a data picker, which will multiply the event weight (including systematics). Can either be one value per event, or have the same shape as the fill.
...
- `only_for_cats`: Opional, a dictionary of the form `{"category_name": ["category_value", ...]}`. If given, fill the histogram only for the given categories (e.g. to save memory). For example, giving `{"channel": ["is_ee", "is_mm"]}` would fill the histogram only for the ee and mumu channels. If only a single value is given, encasing it in a list is optional (i.e. `{"channel": "is_em"}` is accepted).
### Data pickers
